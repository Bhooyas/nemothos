from models import Issue, State
from langgraph.graph import StateGraph, START, END
from nodes import code_present, create_generate_code, create_security_agent, create_performance_agent, create_optimizer, improve_more
...
def get_graph(llm: ChatOllama) -> StateGraph:
    ...
    graph.add_conditional_edges(START, code_present, {"CODE": "generate_code", "SECURITY": "security_agent"})
    ...
    graph.add_conditional_edges("optimizer", improve_more, {"continue": "security_agent", "END": END})
    graph = graph.compile()
    return graph

@click.command()
@click.option("-m", "--max-generation", type=int, default=3, help="Number of times to run the loop")
@click.option("--hard", type=bool, is_flag=True, default=False, help="Use higher model and optimize harder")
@click.option("--reasoning", type=bool, default=True, help="Use model with reasoning")
@click.option("-f", "--file", type=click.Path(exists=True, file_okay=True, dir_okay=False), help="Path of the file to optimize")
...
def main(max_generation, hard, reasoning, file, task, run_id):
    ...
    print(f"State dumped to {run_id}.json")
